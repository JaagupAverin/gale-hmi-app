 ...
 *
 * Issues:
 *  - When running in Linux native_sim, the getopt_long sometimes crashes if used with invalid argument several times
 *    in a row - probably due to some internal state issue- however it doesn't support the 'optreset' flag either so
 *    we can't do much about if for now. Very nice bug - whatever.
 */
